...
public class SpiderEnemyAI : EnemyAI
{
    ...
    public override void findBait()
    {
        ...
        {
            if (Vector3.Distance(transform.position, r.transform.position) < radius && r.name == "Fly(Clone)")
            {
    ...
    public override void generateSoundWave()
    {
        ...
        if (soundWaveCountdown == 0)
        {
            ...
            Sound.PlaySound(Sound.SoundType.spiderScream);
