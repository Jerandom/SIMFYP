...
public class ZombieEnemyAI : EnemyAI
{
    private void Awake()
    {
        setType(Constants.Zombie);
        base.Awake();
    }
    ...
    public override void findBait()
    {
        ...
        {
            if (Vector3.Distance(transform.position, r.transform.position) < radius && r.name == "Meat(Clone)")
            {
    ...
    public override void generateSoundWave()
    {
        ...
        if (soundWaveCountdown == 0)
        {
            ...
            Sound.PlaySound(Sound.SoundType.zombieScream);
